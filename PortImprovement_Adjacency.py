...
selected_model = 3  # 1: max num_of_connections, budget per year; 2: max num_of_connections, budget per cluster; 3: min distance per cluster, budget per year
...
file4 = xlrd.open_workbook(os.getcwd()+"\\devaluation_rate.xlsx")
sheet3 = file4.sheet_by_index(0)
raw_data3 = [[sheet3.cell_value(r, c) for c in range(sheet3.ncols)] for r in range(sheet3.nrows)]
devaluation_rate_data = copy.deepcopy(raw_data3)
...
for i in range(3,sheet1.ncols-1):
    year_name.append(str(fund_data[0][i]))
...
devaluation_rate = devaluation_rate_data[1][0]
    ...
    for i in range(len(container_port_name)):
        for t in range(len(year_name)):
            ...
            for j in range(len(container_port_name)):
                ...
                if i == j:
                    continue
            c.objective.set_linear([('x'+str(container_port_name[i])+","+str(year_name[t]), link_existence * (len(year_name) - 1 - t) / pow((1 + devaluation_rate/100),t))])
            ...
            for g in range(len(port_clusters)):
                if i in port_clusters[g]:
                    ...
                    c.objective.set_linear([('x'+str(container_port_name[i])+","+str(year_name[t]), 
                                             (len(year_name) - 1 - t) / ((1/(len(port_clusters[g])-1) * total_time * pow((1 + devaluation_rate/100),t))))])
                    break
    c.objective.set_sense(c.objective.sense.maximize)
...
for i in range(len(container_port_name)):
    for t in range(len(year_name)):
        worksheet.write(row, investment_effect_per_port_starting_column+t, investment_effect_per_port_per_year[i][t] / pow((1 + devaluation_rate/100),t))
    row += 1
